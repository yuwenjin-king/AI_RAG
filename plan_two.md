...
> ✅ P1 全部完成（③ RBAC；④ 评估/A-B；⑤ 成本管控；⑥ KEDA；⑦ 规模化(向量调优+渲染缓存+k6 压测)；⑧ 安全合规(PII 脱敏+审计+TLS/静态加密/密钥管理文档)；⑨ CI/CD）。
> ✅ P2 全部完成（⑩ GraphRAG：图存储内存+Neo4j、实体抽取 LLM+启发式、图召回第三路入 RRF；⑪ CDC：事件处理+cdc_worker+DB hash 差量连接器）。
> **plan_two 全部 11 项已落地。**
...
| 10 | GraphRAG（知识图谱召回路） | **P2 ✅** | §4.3/§4.4 | 图存储(内存+Neo4j)+实体抽取+图召回第三路 | 大 | Neo4j |
| 11 | CDC 连接器（DB 类数据源增量） | **P2 ✅** | §4.1 | CDC 事件处理+cdc_worker+DB hash 差量连接器 | 中 | Debezium |
